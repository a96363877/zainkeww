...
import { HelpButton } from "@/components/payment/help-button"
  ...
  const openChat = () => {
    if (window.smartsupp) {
      ...
      if (formData.phoneNumber) {
        window.smartsupp("name", `Customer ${formData.phoneNumber}`)
        window.smartsupp("variable", "Phone", formData.phoneNumber)
      }
    }
  }
  ...
  return (
    <div className="max-w-md mx-auto bg-gradient-to-b from-white to-gray-50 min-h-screen" dir="rtl">
      ...
      <div className="p-5">
        ...
        <PaymentButton isSubmitted={isSubmitted} phoneNumber={formData.phoneNumber} onSubmit={handleSubmit} />

        <HelpButton onOpenChat={openChat} />
      </div>
    </div>
  )
}
